...
// import { getToday } from "../../../../class/section03/03-10-rest-api-with-email/backend/utils.js";
...
# "Error: Query root type must be provided." 에러 방지를 위한 것입니다.
  ...
  type Mutation {
    ...
    createTokenOfPhone(phoneInput: PhoneInput!): String
  }
`;
...
const resolvers = {
  ...
  Mutation: {
    createTokenOfPhone: (_, args) => {
      ...
      const isValid = checkValidationPhone(args.phoneInput.phone);
      // 2-2. 휴대폰 번호 자릿수가 맞다면 핸드폰 토큰 4자리 만들기
      if(isValid){
        const token = getToken();
      // 2-3. 만든 토큰을 핸드폰번호에 토큰 전송하기
        sendTokenToSMS(args.phoneInput.phone, token)
        return "인증완료";
